...
const service = services.find(({ title }) => title === 'Mantenimiento eléctrico')!
          ...
          <div className='h-full cols-container items-center'>
            ...
            <div className='w-6-cols sm:w-1/2-cols'>
              <div className='relative'>
                <Icons.Instalations className='w-4/5 sm:w-full lg:w-4/5 h-auto mx-auto lg:mx-0 fill-transparent stroke-accent stroke-[4px] relative' />
                <Icons.Instalations className='w-4/5 sm:w-full lg:w-4/5 h-auto fill-transparent stroke-accent stroke-[4px] absolute inset-x-0 top-0 mx-auto lg:mx-0 scale-110 blur group-hover:scale-[1.15] duration-300 transition-transform' />
              </div>
            </div>
          </div>
        </div>
      </section>
      {service.items.map((serviceItem, key) => (
        <section key={key} className='bg-dot-white/[0.2] relative flex items-center justify-center'>
          <div className='absolute pointer-events-none inset-0 bg-black [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]' />
          <div className='container py-spacing-7 relative'>
            <div className='cols-container'>
              <div className='w-8-cols lg:w-7-cols'>
                ...
                {typeof serviceItem.description === 'string'
                  ? (
                  ...
                  : serviceItem.description && (
                    <div className='mt-spacing-4 space-y-spacing-3'>
                      {serviceItem.description.map((serviceSubItem, subKey) => (
                        <p key={subKey} className='f-display-3 text-balance'>
                          {serviceSubItem}
                        </p>
                      ))}
                    </div>
